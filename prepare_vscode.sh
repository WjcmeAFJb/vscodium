...
# {{{ dance integration
#
# The dance extension ships as a built-in extension overlay but we load it into
# the workbench's own JS thread (no extension host).  This step:
#   1. Strips main/browser/activationEvents from the on-disk manifest so the
#      regular extension host treats it as "manifest-only" — only the
#      contributes (commands/keybindings) get registered, no code is loaded.
#   2. Embeds the JS bundle as a string literal into the workbench source
#      tree so the workbench compiler picks it up and ships it inside the
#      workbench bundle itself.
DANCE_EXT_DIR="vscode/extensions/dance"
...
PYEOF
  ...
  echo "[dance] generating ${DANCE_CONTRIB_DIR}/danceBundle.ts"
  # Pull the manifest from the source overlay so dance's own
  # `vscode.extensions.getExtension('gregoire.dance').packageJSON` lookups still
  # return a sensible shape — independent of the on-disk manifest we just stripped.
  python3 - "${DANCE_EXT_DIR}/out/web-extension.js" "src/stable/extensions/dance/package.json" "${DANCE_CONTRIB_DIR}/danceBundle.ts" <<'PYEOF'
import json, sys
...
with open(out_path, "w", encoding="utf-8") as f:
    f.write("// auto-generated by prepare_vscode.sh — do not edit\n")
...
PYEOF
  ...
  echo "[dance] skipping integration — ${DANCE_EXT_DIR} or ${DANCE_CONTRIB_DIR} not found"
fi
# }}} dance integration
